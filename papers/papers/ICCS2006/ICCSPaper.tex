...
(e.g.\ \cite{Woolf:2003})
but very few services exist for processing or visualizing data.
...
The goal of the Styx Grid Services system is to create remote services that are just as easy to us as local programs.  The basis of the system is the Styx protocol~\cite{Pike:1999} for distributed systems.  In all Styx-based systems {\em all\/} resources (including physical devices, databases and program entry points) are represented as files (analogous to the representation of the mouse as the file {\tt /dev/mouse} in Unix variants).  Styx itself is a file-sharing protocol that can operate over a large number of transports.  Styx forms the core of the Inferno~\cite{Inferno} operating system, in which applications communicate with all resources using Styx, without knowing whether these resources are local or remote.  The basis of the SGS software is a pure-Java implementation of Styx ({\tt http://jstyx.sf.net}).
