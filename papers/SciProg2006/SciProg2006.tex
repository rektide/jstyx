...
Since all resources in a Styx system are represented as files, the Styx command set is very small, consisting of only 13 commands, of which ``open'', ``read'', ``write'' and ``close'' are perhaps the most commonly used.  (The ``read'' operation is analogous to HTTP GET and the ``write'' operation is analogous to HTTP POST.) Whereas in RPC-style Web Services the resources are accessed through a set of methods (i.e.\ an API), Styx resources are accessed through a hierarchy of files, known as a \textit{namespace\/}.  Clients interact with the resource by reading from and writing to these files using the Styx protocol.

\subsection{The JStyx libraries}
We developed an open source, pure-Java implementation of Styx (JStyx, {\tt http://jstyx.sf.net}) and used it as the base for the SGS system.  

\subsection{Styx Grid Services overview}
...
are represented as files, analogous to the representation of the mouse as the file {\tt /dev/mouse} in Unix variants.  Styx is a file-sharing protocol, similar in some respects to NFS, that can operate over a large number of transports such as TCP/IP and UDP.  It forms the core of the Inferno and Plan9 operating systems, %({\tt http://www.vitanuova.com/inferno}), an operating system
in which applications communicate with all resources using Styx, without knowing whether these resources are local or remote (Styx is equivalent to the ``9P2000'' version of the 9P protocol).  We developed an open source, pure-Java implementation of Styx (JStyx, {\tt http://jstyx.sf.net}) and used it as the base for the SGS system.

All resources in Styx systems are represented as a file hierarchy, which is known as a {\em namespace\/}.  We have defined a file hierarchy that represents the interface to a command-line program (Fig.~\ref{fig:sgsnamespace}): this is the SGS namespace.  Clients interact with this program by reading from and writing to the files in this namespace over the network.  For example, the SGS namespace contains an {\tt inputs/} directory, into which clients write the input files that the program will consume. 
...
A JStyx server (such as an SGS server) can operate in two modes: as a standalone daemon and in ``tunnel'' mode, analogous to the modes of other systems such as Subversion (REF).  As a standalone daemon, the server listens on a single port (9092 by convention) and runs persistently as a generic user, allowing clients to authenticate through a proprietary mechanism.  In ``tunnel'' mode, the server is executed as a transient process by clients that have authenticated through other means such as SSH: the server process then runs as the specific authenticated user and exchanges Styx messages with the client through the tunnel.
