...
public class SteeringPanel extends JPanel implements Terminable
{
    ...
    public void addVariableToMap(SteeredVariable var)
    {
        ...
        this.jdata.addSteeredVariableToMenu(var.getName());
    }
    ...
    public class SteeringDialog extends JDialog implements ActionListener
    {
        ...
        {
            super(jdata, "Add steered variable", true);
            ...
            // Changed by Jon: Select the steering file from those available
            // on the server
            JComboBox cmbSteering = new JComboBox();
            SGSInstanceClient instance = jdata.getSGSInstanceClient();
            try
            {
                CStyxFile[] steeringFiles = instance.getSteeringFiles();
                for (CStyxFile file : steeringFiles)
                {
                    cmbSteering.addItem(file);
                }
            }
            ...
            this.add(cmbSteering, "3, 3, 9, 3");
            // End of stuff changed by Jon
            ...
            Dimension parent_dimension = jdata.getSize();
            Dimension dialog_dimension = super.getSize();
            Point parent_location = jdata.getLocation();
        ...
        private boolean setValues()
        {
            ...
            {
                JOptionPane.showConfirmDialog(jdata, //parent defined in outer class
            ...
            if (this.steering_file.isFile())
            {
                ...
                if (JOptionPane.NO_OPTION == JOptionPane.showConfirmDialog(jdata,
                    warning_message,
                    "Confirm overwrite",
    ...
    public class SteeredVariable extends JPanel implements ActionListener, ChangeListener
    {
        ...
        private void createSteeredNumberPanel(Object val, Object min, Object max)
        {
            ...
            if (!min.toString().matches("") && !max.toString().matches(""))
            {
                ...
                { //otherwise, if it is a double type
        ...
        private void writeValueToFile()
        {
            try
            {
                ...
                if (instance != null)
                {
                    ...
                    // Get handle to steering file on Styx server
                    CStyxFile steeringFile = instance.getInstanceRoot().getFile("steering/Pgrad");
                    steeringFile.setContents(output_string);
                }
