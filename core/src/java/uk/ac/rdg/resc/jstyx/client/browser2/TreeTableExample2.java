...
public class TreeTableExample2 {
    ...
    protected StyxFileSystemModel    model;
    ...
    public TreeTableExample2(String hostname, int port) throws StyxException {
	this.path = path;
	ttCount++;
	...
	model = createModel(hostname, port);
     ...
     */
    protected StyxFileSystemModel createModel(String hostname, int port)
        throws StyxException
    {
        StyxConnection session = new StyxConnection(hostname, port);
	return new StyxFileSystemModel(session.getRootDirectory());
    }
	...
	JMenuItem        menuItem;
        ...
        // This will open a new TreeTable rooted at the point selected
	...
	if (args.length == 2) {
        ...
        new TreeTableExample2(host, port);
    }
}
